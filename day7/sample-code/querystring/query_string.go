...
func useHandle(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	name, ok := query["name"]
...
<html>
	...
	<ul>
		...
		{{if .Age}}<li>Age:{{.Age}}{{end}}
	</ul>
	{{else}}
	<p>Data not avaiable</p>
	{{end}}
</html>
`
...
func bookHandle(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	books := []book{
		{ID: 1, Name: "book1", Author: "author1", Year: 2008},
		{ID: 2, Name: "book2", Author: "author2", Year: 2009},
		{ID: 3, Name: "book3", Author: "author3", Year: 2007},
		{ID: 4, Name: "book4", Author: "author4", Year: 20021},
	}
	...
	if ok {
		...
		bookReturn = filterByName(books, filterName)
	} else {
		bookReturn = append(bookReturn, books...)
	}

	year, ok := query["year"]
	if ok {
		filterYear, _ := strconv.Atoi(strings.Join(year, ""))
		bookReturn = filterByYear(bookReturn, filterYear)
	}

	booksJson, err := json.Marshal(bookReturn)
